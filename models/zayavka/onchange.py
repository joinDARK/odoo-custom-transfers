...
class ZayavkaOnchange(models.Model):
    ...
    def _onchange_contragent_id(self):
        """
        Автоматическое заполнение client_id при выборе contragent_id.
        Ищет последние 50 заявок с тем же контрагентом и определяет 
        наиболее часто встречающегося клиента.
        """
        if self.contragent_id and not self.client_id:
            try:
