...
- Верни ТОЛЬКО переведенное название без объяснений, ссылок и комментариев"""
    ...
    def _convert_docx_to_pdf_base64(self, docx_base64, sign_individual=False):
        ...
        try:
            ...
            # Используем LibreOffice для всех документов (без водяных знаков, хорошее качество)
            if sign_individual:
                _logger.info("🎯 Используем LibreOffice для конвертации индивидуального документа")
            else:
                _logger.info("📄 Используем LibreOffice для конвертации документа")
            pdf_base64 = self._convert_docx_to_pdf(docx_bytes)
